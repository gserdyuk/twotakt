...
- [docs/article_5_microservices_ru.md](docs/article_5_microservices_ru.md) — S1/#5 RU master longread draft (publication): two glued decisions, distribution retreats, "new module boundary = the context window", close with the computable-boundary pitch
- [docs/prior_art_landscape.md](docs/prior_art_landscape.md) — prior-art bibliography for series S (two-sided, verified-by-fetch); clusters A-E (phase-3 collapse + microservices/re-verification/options/lifecycle): ~24 sources + 7 classical anchors (Brooks/Conway/Parnas/Dijkstra/Baldwin-Clark/Boehm/Lientz-Swanson); per-cluster + cross-cluster synthesis; confirms twotakt=constraint / 3A8=optimization boundary
...
- 2026-08-04 — cold-reader rule: surface docs open with a definition, before slogan/promise/audience; README lead rewritten, constraint added to CLAUDE.md
