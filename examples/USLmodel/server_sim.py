...
if __name__ == "__main__":
    ...
    # Uncomment to sweep arrival rate and see the degradation curve:
    #
    # print("\n=== Sweep: arrival_rate ===")
    # print(f"  {'rate':>5} {'thr':>7} {'p50':>8} {'p95':>8} {'p99':>8} "
    #       f"{'drop_to':>8} {'drop_buf':>8}")
    # for rate in [1, 2, 4, 6, 8, 10, 12, 14, 16, 18]:
    #     r = run(Config(arrival_rate=rate, sla_seconds=2.0, max_threads=200))
    #     print(f"  {rate:5.1f} {r['throughput_rps']:7.2f} "
    #           f"{(r['latency_p50'] or 0):8.3f} "
    #           f"{(r['latency_p95'] or 0):8.3f} "
    #           f"{(r['latency_p99'] or 0):8.3f} "
    #           f"{r['dropped_timeout']:8d} {r['dropped_buffer']:8d}")
