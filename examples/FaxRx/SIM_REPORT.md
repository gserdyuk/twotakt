...
See `sweep.png` — three panels (success rate, PSTN block rate, p95 effective
latency vs burst), one curve per architecture. The contrast between the block-rate
panel and the latency panel is the visual statement of the report: architecture A
trades a rising block rate for flat, healthy latency (fast, honest failure at the
door), while architecture B shows zero blocking but latency climbing to the OCR SLA
ceiling (slow, hidden failure inside). Generated by `plot_sweep.py` from
`sweep_results.json`.
