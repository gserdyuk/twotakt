"""
Three-panel plot of the arrival-rate sweep — TEMPLATE.

The three-panel layout is the metric checklist's Rule 8 — do not
collapse it to a single chart. The panels are:

  1. throughput vs offered load (with ideal y=x reference)
  2. success rate vs offered load (linear 0-1)
  3. latency percentiles vs offered load (log-y):
       solid  = effective (timeouts counted as latency = SLA)
       dashed = ok-only (the misleading survivorship metric)
     The gap between solid and dashed visualizes survivorship bias.
...
def plot(rows, out_path):
    ...
    ax_thr.set_title("Throughput vs offered load")
